...
move_ubuntu_to_yocto_output(){
	...
	DIR="${RZ_TARGET_DIR:-yocto_rzcmn_board}/build/tmp/deploy/images/${MACHINE}/target/images"
	if [ -d "$DIR" ]; then
	...
	DIR_ROOTFS="${RZ_TARGET_DIR:-yocto_rzcmn_board}/build/tmp/deploy/images/${MACHINE}/target/images/rootfs"
