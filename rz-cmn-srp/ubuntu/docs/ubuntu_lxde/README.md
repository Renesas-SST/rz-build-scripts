# Ubuntu LXDE System development on RZ boards #
This is the quick startup guide for RZ boards to develop on Ubuntu LXDE (with Desktop environment support).

The following sections will describe how to build this custom Ubuntu LXDE image and set up the development environment for all supported RZ boards.

## Status
This is a Custom Ubuntu LXDE release of the RZ development product for **RZ/G2L-SBC**, **RZ/G2L-EVK**, **RZ/V2L-EVK** and **RZ/V2H-EVK**.
...
```shell
IMAGE=ubuntu-lxde ./rz_builder.sh build
```
...
- Upon logging in for the first time, the system will automatically use LXDE as the desktop environment.
- No manual selection is required, ensuring a seamless user experience.

This default setting ensures that users immediately benefit from the lightweight and responsive features of LXDE without any additional configuration.
...
- The taskbar may not display the correct screen width on some systems. Workaround: Open terminal on LXDE, and run the command `lxpanelctl menu`, then change the Width to 0%.
