...
build_sdk() {
	...
	case "${IMAGE}" in
		"all-supported-images")
			...
			MACHINE=rzpi DISTRO=ubuntu-tiny bitbake renesas-ubuntu -c populate_sdk_ext
		...
		"all-supported-images")
			...
			${JQ} -r --arg key "ubuntu" '.[$key][]' "${IMAGES_JSON}" | while IFS= read -r img; do
				...
				sudo IMAGE="${img}" RZ_TARGET_DIR="$RZ_TARGET_DIR" bash -c '
						...
						check_yocto_artifacts "${RZ_TARGET_DIR}/build/tmp/deploy/images/rzpi/target/images/rootfs"
...
deploy_build_assets() {
	...
	local target_dir="${RZ_TARGET_DIR}/build/tmp/deploy/images/rzpi/host/src/rz-cmn-srp"
...
deploy_ubuntu_build_assets() {
	...
	local target_dir="${RZ_TARGET_DIR}/build/tmp/deploy/images/rzpi/host/src/rz-cmn-srp/ubuntu"
...
output() {
	...
	cd "${OUTPUT}" || { log_error "Failed to switch to ${OUTPUT}" ; exit 1; }
